...
namespace LanguageIndicator
{
    ...
    internal sealed class OsdWindow : NativeWindow, IDisposable
    {
        ...
        public void ShowState(InputState state, Rectangle anchor, bool isCaret)
        {
            ...
            using (Bitmap bmp = Render(state, _scale, Theme.Get(Settings.ThemeName), Settings.Opacity))
            {
                SetBitmap(bmp);
            }
        ...
        private static Bitmap Render(InputState state, float s, Theme th, int opacity)
        {
            float op = opacity / 100f;
            Color fill = Color.FromArgb((int)Math.Round(255 * op), th.Fill);
            Color bar = state.IsAccent ? (th.Bar.IsEmpty ? Theme.SystemAccent() : th.Bar) : Color.FromArgb(100, th.Text);
            ...
            string glyph = state.Glyph, caption = state.Caption;
            ...
            using (var glyphFont = MakeFont(cjkGlyph ? "Microsoft YaHei UI" : (th.LatinFont ?? "Segoe UI Semibold"),
                                            (cjkGlyph ? 22f : 21f) * s, cjkGlyph ? FontStyle.Bold : FontStyle.Regular))
            ...
            using (var mg = Graphics.FromImage(measureBmp))
            {
                ...
                var bmp = new Bitmap((int)Math.Ceiling(w) + 2 * m, (int)Math.Ceiling(h) + 2 * m, PixelFormat.Format32bppArgb);
                using (var g = Graphics.FromImage(bmp))
                {
                    ...
                    {
                        // Soft shadow outside the pill only (so the glass stays clean), fainter when transparent.
